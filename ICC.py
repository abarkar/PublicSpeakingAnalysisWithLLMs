...
for pair in model_with_test_number:
    rater_count+=1
    csv_filename = f'dimentions_all_files_{dimension}_evaluation_prompt_{version}_test_{pair[1]}_LLM_answers_{pair[0]}_7b.csv'
    ...
    # predicted_df = predicted_df[(predicted_df['category']=="full")&(~predicted_df['ID'].str.lower().isin(forbiden_ID))]
    print("Considered daya: ", np.shape(predicted_df))
...
merged_df = merged_df[['ID', 'category', 'rater', 'score']]
...
for index, row in ground_truth_df.iterrows():
    ground_truth_data.append({
        'ID': str(row['presentation']).lower(),
        'category': row['video'],
        'score': row[dimension + '_rms'],
        'rater': rater_count  # Assign rater value for ground truth
    })
ground_truth_df_full = pd.DataFrame(ground_truth_data)
ground_truth_df_full=ground_truth_df_full[ground_truth_df_full['category']=="full"]
...
# Save ICC results to CSV file
output_icc_filename = f'ICC_{dimension}.csv'
output_icc_path = os.path.join(csv_folder, output_icc_filename)
# Calculate ICC for each slice
slices = merged_df.groupby('category')
# icc_results = {}
...
# llama3 models only
calculate_pairwise_icc(merged_df, [1, 2, 3], dimension, csv_folder, "only_llama3")  # Adjust the rater numbers accordingly
